...
def obter_acoes_ibrx100():
    """Obtém ações do IBrX100 diretamente da B3"""
    url = "https://bvmf.bmfbovespa.com.br/indices/ResumoCarteiraTeorica.aspx?Indice=ibrx"
    try:
        response = requests.get(url, timeout=30)
        response.raise_for_status()
        
        # Usa BeautifulSoup para extrair códigos de ações
        soup = BeautifulSoup(response.text, 'html.parser')
        
        # Procura por células <td> que contêm códigos de ações
        codigos = []
        for td in soup.find_all('td'):
            texto = td.get_text().strip()
            # Verifica se é um código de ação (3-5 letras + 1-2 números)
            if re.match(r'^[A-Z]{3,5}[0-9]{1,2}$', texto):
                codigos.append(f"{texto}.SA")
        
        # Remove duplicatas
        codigos = list(set(codigos))
        
        print(f"Obtidas {len(codigos)} ações do IBrX100")
        return codigos
        
    except Exception as e:
        print(f"Erro ao obter ações do IBrX100: {e}")
        return []
